...
L’apporto di modifiche al file [!DNL Log Processing Mode.cfg], inclusa l’aggiunta o la rimozione di origini, non comporta la rielaborazione dei dati.
...
1. Quando lavori nel profilo del set di dati, apri il [!DNL Profile Manager] e fai clic su **[!UICONTROL Dataset]** per visualizzarne il contenuto.
   ...
   >[!NOTE]
   >
   >Se il file [!DNL Log Processing Mode.cfg] non si trova nella directory del profilo desiderato, è necessario copiare questo file dalla directory Base sul computer server di Data Workbench nella directory del profilo.

   Per informazioni sull&#39;apertura e l&#39;utilizzo di [!DNL Profile Manager,], vedere la *Guida utente Data Workbench*.

1. Fai clic con il pulsante destro del mouse sul segno di spunta accanto al nome del file di configurazione e fai clic su **[!UICONTROL Make Local]**. Un segno di spunta per questo file viene visualizzato nella colonna [!DNL User].
...
1. Modifica i parametri nel file di configurazione utilizzando la seguente tabella come guida.
   ...
   <tr> 
      ...
      <td colname="col2"> <p>Parametro di ottimizzazione che specifica il rapporto tra byte di log totali e non letti in cui il sistema entra in modalità <span class="wintitle"> Fast Input</span> (e successivamente <span class="wintitle"> Fast Merge</span>) invece di elaborare i dati in tempo reale. </p> <p> Il valore predefinito è 200, il che significa che il sistema entra in modalità <span class="wintitle"> Fast Input</span> dalla modalità in tempo reale quando i dati di log non letti sono a 1/200 del totale dei dati. Un rapporto di decisione più elevato rende il sistema entrare più facilmente in modalità <span class="wintitle"> Fast Input</span>, mentre un rapporto più basso rende meno probabile entrare in modalità <span class="wintitle"> Fast Input</span>. </p> <p> <p>Nota: L'impostazione del parametro su 0 impedisce al sistema di entrare in modalità <span class="wintitle"> Fast Input</span>, anche per l'elaborazione iniziale. L'impostazione del parametro su 1.1 consente al sistema di immettere <span class="wintitle"> Fast Input</span> durante l'elaborazione iniziale, ma non per l'elaborazione successiva. Adobe sconsiglia di utilizzare valori compresi tra 0 e 1.1. Per ulteriori informazioni sull'impostazione di questo parametro, contattare Adobe. </p> </p> </td> 
   </tr> 
   <tr> 
   ...
   <tr> 
      ...
      <ul id="ul_569B90E9A85246F88906FA5444F8A93E"> 
       <li id="li_3EF182CEF4A44106B5267175EC62B9AB"> Fai clic con il pulsante destro del mouse su <span class="uicontrol"> Origini offline</span>, quindi fai clic su <span class="uicontrol"> Aggiungi nuovo</span> &gt; <span class="uicontrol"> Origine</span>. </li> 
       <li id="li_E8FBA212F4784B1A830745A90BB3AF90"> Nel parametro per la nuova sorgente, immetti la maschera della sequenza di log. Per le sorgenti di registro Sensor con nomi di file in formato YYYYMMDD-<i>SENSORID</i>.vsl, la maschera è <i>SENSORID.SENSORID</i> sensibile a maiuscole e minuscole. Per le origini dei file di log, la maschera è la stringa estratta dal <span class="wintitle"> Modello maschera</span>. Vedere <a href="../../../home/c-dataset-const-proc/c-log-proc-config-file/c-log-sources.md#concept-3d4fb817c057447d90f166b1183b461e"> File di registro</a>. </li> 
   ...
   Quando modifichi il file [!DNL Log Processing Mode.cfg] all’interno di una finestra di Data Workbench, puoi utilizzare i tasti di scelta rapida per le funzioni di modifica di base, tra cui Taglia (Ctrl+x ), Copia (Ctrl+c) , Incolla (Ctrl+v ), Annulla (Ctrl+z ), Ripristina (Ctrl+Maiusc+Z ), seleziona la sezione (clic+trascina) e seleziona tutto (Ctrl+a ). Inoltre, è possibile utilizzare le scelte rapide per copiare e incollare il testo da un file di configurazione ( [!DNL .cfg]) a un altro.
...
1. In [!DNL Profile Manager], fai clic con il pulsante destro del mouse sul segno di spunta per il file nella colonna [!DNL User], quindi fai clic su **[!UICONTROL Save to]** > **[!UICONTROL datasetprofile name]**.
